...
if [[ "${OUTPUT_PLOTTING}" == true ]]; then
    for plot_year in $(seq $START_YEAR $END_YEAR);
        do asli plot $DATA_DIR/era5_mean_sea_level_pressure_monthly_*.nc -y $plot_year -i $OUTPUT_DIR/asli_calculation_$FILE_IDENTIFIER.csv -o $OUTPUT_DIR/asli_plot_$plot_year.png;
    done
fi
